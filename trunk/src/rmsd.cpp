...
{
    const size_t ncoords = s.get_ncoords();
...
{
    assert(target.get_ncoords() == reference.get_ncoords());
    const size_t NCOORDS = target.get_ncoords();    
...
{
    assert(a.get_ncoords() == b.get_ncoords());
    
    const size_t NCOORDS = a.get_ncoords();    
    RMSD  ret(0);
